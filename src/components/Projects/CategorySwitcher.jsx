...
const CategorySwitcher = ({
  ...
  onCategoryChange,
  onSubcategoryChange,
  onFilterProjects, // Make sure this prop is passed and is a function
}) => {
  const [activeSubcategories, setActiveSubcategories] = useState([]);

  useEffect(() => {
    // Ensure selectedCategory exists before trying to find subcategories
    if (selectedCategory) {
      const categoryData = categories.find(
        (item) => item.name === selectedCategory
      );
      setActiveSubcategories(categoryData?.subcategories || []);
    } else {
      setActiveSubcategories([]); // Reset when no category is selected
    }
  ...
  const handleCategoryChange = (category) => {
    ...
    if (onFilterProjects) {
      onFilterProjects(category, null);
    }
  };
  ...
  const handleSubcategoryChange = (subcategory) => {
    ...
    if (onFilterProjects) {
      onFilterProjects(selectedCategory, subcategory);
    }
  };
